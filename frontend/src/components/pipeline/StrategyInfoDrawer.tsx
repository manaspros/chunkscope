...
function StatBadge({ label, value, color }: { label: string; value: string; color: string }) {
    return (
        <div className="flex flex-col items-center gap-1 p-2 rounded-lg bg-white/[0.03] border border-white/[0.06]">
            <span className="text-[8px] uppercase font-bold text-neutral-500">{label}</span>
                        ...
                        <ScrollArea className="flex-1">
                            <div className="p-5 space-y-5">
                                ...
                                {strategy.when_to_use.length > 0 && (
                                    <div>
                                        <h4 className="text-[10px] uppercase font-bold text-neutral-500 mb-2">When to Use</h4>
                                ...
                                {strategy.when_not_to_use.length > 0 && (
                                    <div>
                                        <h4 className="text-[10px] uppercase font-bold text-neutral-500 mb-2">When NOT to Use</h4>
                                        <ul className="space-y-1.5">
                                ...
                                {strategy.best_for.length > 0 && (
                                    <div>
                                        <h4 className="text-[10px] uppercase font-bold text-neutral-500 mb-2">Best For</h4>
                                        <div className="flex flex-wrap gap-1.5">
                                            {strategy.best_for.map((tag, i) => (
                                                <Badge key={i} variant="outline" className="text-[10px] border-white/10 text-neutral-300">
                                                    {tag}
                                                </Badge>
                                            ))}
                                        </div>
                                ...
                                {strategy.tradeoffs && (
                                    <div>
                                        <h4 className="text-[10px] uppercase font-bold text-neutral-500 mb-2 flex items-center gap-1">
                                            <ArrowRightLeft className="w-3 h-3" /> Tradeoffs
                                        </h4>
                                        <p className="text-xs text-neutral-400 leading-relaxed">{strategy.tradeoffs}</p>
                                    </div>
                                )}
                                ...
                                {strategy.pairs_well_with.length > 0 && (
                                    <div>
                                        <h4 className="text-[10px] uppercase font-bold text-neutral-500 mb-2">Pairs Well With</h4>
                                        <div className="flex flex-wrap gap-1.5">
                                            {strategy.pairs_well_with.map((id, i) => (
                                                <button
                                                    key={i}
                                                    onClick={() => onSelectStrategy?.(id)}
                                                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-medium bg-white/[0.05] border border-white/[0.08] text-neutral-300 hover:bg-white/[0.08] hover:text-white transition-colors cursor-pointer"
                                                >
                                ...
                                {strategy.example_config && Object.keys(strategy.example_config).length > 0 && (
                                    <div>
                                        <div className="flex items-center justify-between mb-2">
                                            <h4 className="text-[10px] uppercase font-bold text-neutral-500">Example Config</h4>
                                            <button
                                        ...
                                        <pre className="text-[10px] text-neutral-400 font-mono p-3 rounded-lg bg-white/[0.02] border border-white/[0.06] overflow-x-auto whitespace-pre-wrap">
                                            {JSON.stringify(strategy.example_config, null, 2)}
                                        </pre>
                                        {onApplyConfig && (
                                            <Button
                                                size="sm"
                                                className="w-full mt-2 h-8 text-xs bg-white/[0.06] hover:bg-white/[0.1] text-white border border-white/[0.08]"
                                ...
                                {strategy.decision_factors && strategy.decision_factors.length > 0 && (
                                    <div>
                                        <h4 className="text-[10px] uppercase font-bold text-neutral-500 mb-2">Decision Factors</h4>
                                        <ul className="space-y-1">
                                            {strategy.decision_factors.map((factor, i) => (
                                                <li key={i} className="text-[10px] text-neutral-400 flex items-start gap-1.5">
                                                    <DollarSign className="w-3 h-3 text-neutral-600 mt-0.5 shrink-0" />
                                                    {factor}
                                                </li>
                                            ))}
                                        </ul>
